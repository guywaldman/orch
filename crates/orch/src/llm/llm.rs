...
pub struct TextCompleteStreamResponse {
    pub stream: Pin<Box<dyn Stream<Item = String> + Send>>, // TODO: Handle context with streaming response.
                                                            // pub context: Vec<i64>,
}
...
//         response: &str,
//         type_name: &str,
//     ) -> Result<TOption, LlmOrchestrationError>;
...
//             .map_err(|e| LlmOrchestrationError::Parsing(e.to_string()))?;
...
//             .as_str()
//             .ok_or(LlmOrchestrationError::Parsing(
//                 "Response from LLM does contains a 'type' field, but it is not a string"
//                     .to_string(),
//             ))?;
...
//                 response: &str,
//                 type_name: &str,
//             ) -> Result<TestOption, LlmOrchestrationError> {
//                 match type_name {
//                     "city" => Ok(TestOption::City(
//                         serde_json::from_str::<CityOptionVariant>(response).unwrap(),
//                     )),
//                     "country" => Ok(TestOption::Country(
//                         serde_json::from_str::<CountryOptionVariant>(response).unwrap(),
//                     )),
//                     _ => Err(LlmOrchestrationError::Parsing(format!(
//                         "Unknown type name: {}",
//                         type_name
//                     ))),
//                 }
